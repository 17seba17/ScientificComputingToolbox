...
void RawDataJSON::readTable() {
  ...
  for (auto &array_element : pt) {
    for (auto &property : array_element.second) {
      ...
      if (type != Detail::ExpectedType::Any) {
        ...
        double categorical =
            (type == Detail::ExpectedType::String) ? true : false;
  ...
  for (auto &array_element : pt) {
    for (auto const &column : (*infoColumns_)) {
      ...
      auto type = column.isCategorical_ ? Detail::ExpectedType::String
                                        : Detail::ExpectedType::Double;

      auto it =
          std::find_if(array_element.second.begin(), array_element.second.end(),
