...
template <std::size_t size> class ODE {
protected:
  ...
  ODE(const ButcherTableau &bt, const FunctionsArray<size> &fa,
...
public:
  void simulate(double dt, int num_steps, const std::string &output_filename) {
    ...
    if (!outfile.is_open()) {
      throw std::runtime_error("Error: Impossible to open the output file: " +
                               output_filename);
    }
...
template <std::size_t size> class ForwardEuler : public ODE<size> {
public:
  ForwardEuler(const FunctionsArray<size> &fa, const Eigen::VectorXd &status,
               double time)
      : ODE<size>(ButcherTableau({}, std::vector<double>{1.0}), fa, status,
                  time) {}
};

template <std::size_t size> class RK4 : public ODE<size> {
public:
  RK4(const FunctionsArray<size> &fa, const Eigen::VectorXd &status,
      double time)
      : ODE<size>(ButcherTableau({0.5, 0, 0.5, 0, 0, 1},
                                 std::vector<double>{0.1666666, 0.3333333,
                                                     0.333333, 0.1666666}),
                  fa, status, time) {}
};

template <std::size_t size> class Rule_3_8 : public ODE<size> {
public:
  Rule_3_8(const FunctionsArray<size> &fa, const Eigen::VectorXd &status,
           double time)
      : ODE<size>(
            ButcherTableau({0.3333333, -0.3333333, 1, 1, -1, 1},
                           std::vector<double>{0.125, 0.375, 0.375, 0.125}),
            fa, status, time) {}
};

template <std::size_t size> class SecondOrder : public ODE<size> {
public:
  SecondOrder(const FunctionsArray<size> &fa, const Eigen::VectorXd &status,
              double time, double alpha)
      : ODE<size>(
            ButcherTableau({alpha}, std::vector<double>{1 - 1 / (2 * alpha),
                                                        1 / (2 * alpha)}),
            fa, status, time) {}
};

template <std::size_t size> class Midpoint : public SecondOrder<size> {
public:
...
template <std::size_t size> class Heun : public SecondOrder<size> {
public:
...
template <std::size_t size> class Ralston : public SecondOrder<size> {
public:
